...
library BitcoinAddressEncoder {
    ...
    function encodeBase58Check(bytes memory data, uint8 version) internal pure returns (string memory) {
        ...
        for (uint256 i = 0; i < extended.length; i++) {
            uint256 carry = uint8(extended[i]);
            for (uint256 j = 0; j < digitsLength; j++) {
     ...
     */
    function encodeBech32(uint8 witnessVersion, bytes memory data, string memory hrp)
        internal
        pure
