...
library LibTaproot {
    ...
    bytes32 public constant TAP_LEAF_HASH = 0xaeea8fdc4208983105734b58081d1e2638d35f1cb54008d4d357ca03be78e9ee;
    bytes32 public constant TAP_BRANCH_HASH = 0x1941a1f2e56eb95fa2a9f194be5c01f7216f33ed82b091463490d05bf516a015;
    bytes32 public constant TAP_TWEAK_HASH = 0xe80fe1639c9ca050e3af1b39c143c63e429cbceb15d940fbb5c5a1f4af57c5e9;
    ...
    function taggedHashBranch(bytes memory data) internal pure returns (bytes32) {
        return precomputedTaggedHash(TAP_BRANCH_HASH, data);
    }

    function taggedHashTweak(bytes memory data) internal pure returns (bytes32) {
        return precomputedTaggedHash(TAP_TWEAK_HASH, data);
    }

    function precomputedTaggedHash(bytes32 tagHash, bytes memory data) internal pure returns (bytes32) {
        return sha256(bytes.concat(tagHash, tagHash, data));
    }

    function taggedHash(bytes memory tag, bytes memory data) internal pure returns (bytes32) {
    ...
    function verifyTaprootScriptPubKey(bytes32 spk, bytes calldata script, bytes32[] calldata merkleProof, uint256 PX)
        public
        pure
        returns (bool)
    {
        bytes32 mastRoot = computeMastRootFromMerkleProof(script, merkleProof);
        uint256 tweakValue = uint256(taggedHashTweak(bytes.concat(abi.encodePacked(PX), mastRoot)));
