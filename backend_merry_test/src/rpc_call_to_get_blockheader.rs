...
pub async fn call_rpc_for_blockheaders(
    start_height: u64,
    end_height: u64,
    ...
    for height in start_height..=end_height {
        ...
        let blockhash = parsed_response.result.as_str().unwrap_or("").to_string();

        if blockhash.is_empty() {
            eprintln!("Failed to get block hash for height: {}", height);
            continue;
        }
        ...
        if let Some(header) = block_data {
            ...
            println!("Successfully saved block header for height {}", height);
        } else {
            eprintln!("Failed to extract block header for height {}", height);
        }
    }
